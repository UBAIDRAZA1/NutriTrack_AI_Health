...
const Auth = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const withTimeout = (p, ms) => new Promise((resolve, reject) => {
        const t = setTimeout(() => reject({ code: "auth/network-timeout", message: "Request timed out" }), ms);
        p.then((v) => { clearTimeout(t); resolve(v); }).catch((e) => { clearTimeout(t); reject(e); });
      });
      const result = isLogin
        ? await withTimeout(loginUser(payload), 12000)
        : await withTimeout(signupUser(payload), 12000);
      ...
      if (isLogin) {
        ...
        setIsLogin(true);
        navigate("/", { state: { success: "Account created! Please login" } });
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-tr from-black via-gray-900 to-black">
      {success && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 bg-emerald-600 text-white px-4 py-2 rounded-lg shadow-lg">
          {success}
        </div>
      )}
